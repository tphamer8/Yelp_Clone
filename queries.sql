...
AND (
    (a.attribute = 'BusinessAcceptsCreditCards' AND a.attribute_value = 'True')
    OR (a.attribute = 'RestaurantsPriceRange2' AND a.attribute_value = 'True')
    OR (a.attribute = 'WiFi' AND a.attribute_value = 'free')
)
...
BEGIN
    ...
    RETURN R * 2 * atan2(sqrt(sin(difLat / 2) ^ 2 + cos(radians(lat1)) * cos(radians(lat2)) * sin(difLong / 2) ^ 2), sqrt(1 - (sin(radians(lat2 - lat1) / 2) ^ 2 + cos(radians(lat1)) * cos(radians(lat2)) * sin(difLong / 2) ^ 2)));
END;
$$ LANGUAGE plpgsql;
-- Test Example
